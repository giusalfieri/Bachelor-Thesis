...
The thesis studies **Denoising Diffusion Probabilistic Models** (DDPMs), the class of generative models introduced by Ho, Jain and Abbeel (2020) that underlies text-to-image systems such as Stable Diffusion, Imagen and DALL·E 2. The treatment is deliberately self-contained: every object is defined before it is used, the derivation of the training objective is carried out step by step, and the required background in probability, Markov chains, variational inference and the U-Net architecture is supplied in appendices rather than assumed.
...
> [!IMPORTANT]
> **Scope.** This is a work of exposition and analysis rather than of experimentation. No novel architecture is proposed, and no model is trained or benchmarked. Empirical results such as FID scores are cited from the literature and attributed as such.

---

## What the thesis establishes

**Positioning within generative modelling.** Generative modelling is contrasted with discriminative modelling, and a taxonomy of maximum-likelihood generative models is developed following Goodfellow (2016) and Foster (2023), according to how each family represents the density $`p_{\boldsymbol{\theta}}(\mathbf{x})`$: explicitly and tractably, explicitly but approximately, or implicitly. Within this taxonomy DDPMs belong to the models that optimise an explicit approximation, namely a variational lower bound (Chapter 1).
...
**Derivation of the training objective.** Starting from the intractable negative log-likelihood, the variational bound is obtained by specialising the VAE lower bound of Kingma and Welling to the diffusion setting, then decomposed term by term and reduced to the simplified noise-prediction loss minimised in practice. The full chain of equalities is reproduced, with the justification of each step stated (Appendix B).

**Mathematical prerequisites.** Probability distributions, expectation and variance, statistical independence, the Gaussian and multivariate Gaussian, Markov chains and maximum-likelihood estimation are collected in Appendix A. The U-Net of Ronneberger et al. (2015), including the down/up-sampling blocks and skip connections, is described in Appendix C. The thesis is therefore readable without external references.
...
The forward (or *diffusion*) process $`q`$ is a **fixed** Markov chain, containing no learnable parameters, that gradually adds Gaussian noise to $`\mathbf{x}_0`$ over $`T`$ timesteps. By the Markov property the joint law of the trajectory factorises as
...
The scaling factor $`\sqrt{1-\beta_t}`$ is what keeps the variance of the chain bounded, so that the marginals converge rather than diverge.
...
This identity is what makes training feasible: a single timestep $`t`$ may be sampled uniformly and its loss evaluated in constant time.
...
The Gaussian form is justified by the behaviour of the true reverse conditional: when the $`\beta_t`$ are small, that conditional is itself approximately Gaussian, so the family is well matched to the target. Ho et al. further fix the covariance to $`\boldsymbol{\Sigma}_{\boldsymbol{\theta}}(\mathbf{x}_t,t) = \sigma_t^2 \mathbf{I}`$ with $`\sigma_t^2`$ untrained, leaving only the mean to be learned.
...
This posterior is what makes the variational bound computable, since it supplies a Gaussian *target* against which each learned transition can be compared in closed form rather than by Monte Carlo estimation.
...
Appendix B reproduces this derivation in full, then decomposes $`L_{\mathrm{vlb}}`$ into per-timestep terms, each of which is a Kullback–Leibler divergence between two Gaussians and therefore admits a closed form:
...
with $`\boldsymbol{\epsilon} \sim \mathcal{N}(\mathbf{0},\mathbf{I})`$ and $`t \sim \mathcal{U}\{1,\dots,T\}`$. Because of the dropped weighting, $`L_{\mathrm{simple}}`$ is not the variational bound itself but a reweighting of it, one that empirically improves sample quality.

The generative problem is thereby reduced to a supervised regression: predicting, from a noisy image and a timestep, the noise that produced it. Ho et al. make explicit the correspondence between this objective and denoising score matching.
...
The document is a `book`-class LaTeX project using `biblatex` with the **biber** backend, and requires shell-escape for the `svg` package. An [`arara`](https://islandoftex.github.io/arara/) directive block at the top of `main.tex` encodes the full build:
...
Two final passes are required for cross-references, the table of contents and the bibliography to settle. Compilation also requires **Inkscape** on the `PATH`, since `svg` converts the `.svg` figures at build time.
